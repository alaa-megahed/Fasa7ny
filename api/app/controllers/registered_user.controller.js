				...
				Business.findByIdAndUpdate(businessID, { $push: { "subscribers": user_found } }, { safe: true, upsert: true, new: true },
					function (err, business) {
						if (err)
							...
							user_found.save();
							// record subscription in statistics
			...
			if (check == 1) {
				Business.findOne({ _id: businessID }, function (err, business) {
					...
					if (user_found) {
						...
						//removing from subscriptions statistics
						StatsController.addStat(new Date(), businessID, 'subscriptions', -1);
...
console.log(req.params.rate);
		...
		if (req.user && req.user instanceof User && req.params.bid && req.params.rate) {
							...
							if (err) {
								...
								Business.findOne({ _id: businessID }, function (err, updatedBusiness) {
									if (err) console.log("error");
										...
										// StatsController.addStat(new Date(), businessID, 'rating', updatedBusiness.average_rating);
