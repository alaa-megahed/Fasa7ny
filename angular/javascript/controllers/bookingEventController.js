...
app.controller('bookingEventController', function($scope, $http,$routeParams,Business,$location,Global,Event,status, Offers,viewOccurences)
{
    ...
    $scope.current_event = $routeParams.id;
      Event.get($scope.current_event).then(
        ...
        $scope.book_cash = function()
        {
          ...
          if($scope.type == 1)
          {
             $http.post('http://54.187.92.64:3000/bookings/createRegUserBookings', {count: $scope.formData.count ,event: $scope.event_occ._id, charge: $scope.charge,business_id:$scope.business_id})
          ...
          {
             $http.post('http://54.187.92.64:3000/bookings/book_event', {count: $scope.formData.count ,event_id: $scope.event_occ._id, charge: $scope.charge, user_id: $scope.user._id})
                    .then(function successCallback(responce){
                      console.log(responce.data);
                      $location.path('/success/'+responce.data._id);
          ...
          token: function(token)
          {
            ...
            $http.post('http://54.187.92.64:3000/bookings/charge', {stripeToken: token.id, amount: $scope.stripe_charge})
                    .then(function successCallback(responce){
                      ...
                      $http.post('http://54.187.92.64:3000/bookings/createRegUserBookings/', {count: $scope.formData.count ,event: $scope.event_occ._id, charge: $scope.charge, stripe_charge: responce.data.id, user_id: $scope.user._id, business_id: $scope.business_id})
                            .then(function successCallback(responce){
                              console.log(responce.data);
                              $location.path('/success/'+responce.data._id);
